import iconCapacityLight from "../assets/Locations/Icons/iconCapacityLight.svg";
import iconCapacityDark from "../assets/Locations/Icons/iconCapacityDark.svg"; 
import iconRubLight from "../assets/Locations/Icons/icon1.svg";
import iconRubDark from "../assets/Locations/Icons/icon3.svg";
export const restraunts = [
    {
		id: "nomad",
		special: "nomad",
		title: 'NOMAD',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Nomad.png`,
		text: 'Самые вкусные и фотогеничные завтраки в Петербурге. У нас вы можете попробовать шакшуку, омлет с крабом или наш фирменный скрембл с мортаделлой или форелью.',
		subText1: 'доступно',
		subText2: '80 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'NOMAD — культовое место для гурманов, где утро начинается с лучших завтраков в городе. Наше меню это европейский comfort food в сердце Петроградской стороны.',
		cardIcon1Desktop: iconRubLight,
		cardIcon2Desktop: iconCapacityLight,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 3 500 ₽',
		price: 'от 3 500 руб',
		tags: [
			{name: "#европейская" ,},
			{name: "#завтраки" ,}
		],
		insideText1: "Каждое блюдо в NOMAD — это диалог традиций и новаторства. В меню вы найдете как узнаваемые вкусы, так и неожиданные сочетания, где главную роль играют качественные сезонные ингредиенты. Сомелье поможет подобрать идеальное вино из нашей обширной коллекции.",
		insideText2: "Интерьер ресторана — это гармоничное сочетание натурального дерева, латуни, стекла и мрамора. Особую атмосферу создает свет: каждый светильник — винтажный экземпляр 60-70-х годов, тщательно подобранный командой NOMAD.",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Nomad/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Nomad/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Nomad/icon_3.svg`,
        },
		subIconsText: [
			"Авторские смузи и кофе",
			"Сезонные продукты",
			"Интерпретации европейской классики"
		]
	},
	{
		id: "oldManKhinkalych",
		special: "oldManKhinkalych",
		title: 'СТАРИК ХИНКАЛЫЧ',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Old-man-Khinkalych.png`,
		text: 'Вас ждут самолепные хинкали, разнообразные виды хачапури, аджапсандал, а также множество блюд, приготовленных по традиционным рецептам. ',
		subText1: 'доступно',
		subText2: '40 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'Старик Хинкалыч — популярное заведение, специализирующееся на грузинской кухне. Основная гордость ресторана — хинкали, разнообразные по начинкам.',
		cardIcon1Desktop: iconRubDark,
		cardIcon2Desktop: iconCapacityDark,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 2 000 ₽',
		price: 'от 2 000 руб',
		tags: [
			{name: "#грузинская" ,},
			{name: "#семейный" ,}
		],
		insideText1: "Грамотно подобранные вина из различных регионов Грузии подчеркнут вкус каждого угощения. Старик Хинкалыч часто принимает участие в кулинарных фестивалях и мастер-классах, где гости могут узнать секреты приготовления грузинских деликатесов.",
		insideText2: "Уютная атмосфера и аутентичный интерьер создают ощущение грузинского дома. Здесь можно насладиться как деловыми обедами, так и дружескими встречами. ",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Old-man-Khinkalych/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Old-man-Khinkalych/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Old-man-Khinkalych/icon_3.svg`,
        },
		subIconsText: [
			"Домашнее вино",
			"Национальные блюда",
			"Грузинская кухня"
		]
	},
	{
		id: "marsala",
		special: "marsala",
		title: 'MARSALA',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Marsala.png`,
		text: 'Авторские блюда по разумным ценам, эксклюзивные вина по бокалам и комфортная обстановка с живыми растениями.',
		subText1: 'доступно',
		subText2: '60 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'Marsala — современное заведение, сочетающее в себе элементы итальянской кухни и уютную атмосферу.',
		cardIcon1Desktop: iconRubDark,
		cardIcon2Desktop: iconCapacityDark,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 1 500 ₽',
		price: 'от 1 500 руб',
		tags: [
			{name: "#европейская" ,},
			{name: "#авторская" ,},
			{name: "#завтраки" ,}
		],
		insideText1: "Тщательно отобранная винная карта с лучшими итальянскими винами дополняет гастрономический опыт. Marsala регулярно проводит тематические вечера и кулинарные мастер-классы, где гости могут научиться готовить традиционные итальянские блюда.",
		insideText2: "Элегантный интерьер и теплая атмосфера делают ресторан идеальным местом для романтических ужинов, деловых встреч и семейных праздников.",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Marsala/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Marsala/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Marsala/icon_3.svg`,
        },
		subIconsText: [
			"Блюда на дровяной печи",
			"Сырное ассорти",
			"Паста по старинным рецептам"
		]
	},
	{
		id: "eggplant",
		special: "eggplant",
		title: 'БАКЛАЖАН',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Eggplant.png`,
		text: 'Это идеальное место для дружеских встреч, где можно насладиться блюдами в непринуждённой обстановке, а семейные обеды становятся настоящим праздником.',
		subText1: 'доступно',
		subText2: '40 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'Баклажан — уютный ресторан, посвященный средиземноморской кухне с акцентом на блюда из свежих овощей, морепродуктов и оливковых масел.',
		cardIcon1Desktop: iconRubLight,
		cardIcon2Desktop: iconCapacityLight,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 1 000 ₽',
		price: 'от 1 000 руб',
		tags: [
			{name: "#семейный" ,},
			{name: "#европейская" ,},
			{name: "#детскоеменю" ,}
		],
		insideText1: "Наши повара ежедневно создают кулинарные шедевры, сочетая традиционные рецепты с современными техниками подачи. В винную карту вошли лучшие образцы из Италии, Франции и Греции.",
		insideText2: "Просторный зал с панорамными окнами, терракотовой плиткой и живыми оливковыми деревьями создает атмосферу курортного ресторана где-то на побережье. Идеальное место для тех, кто ценит качественную кухню и непринужденную обстановку.",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Eggplant/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Eggplant/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Eggplant/icon_3.svg`,
        },
		subIconsText: [
			"Собственные оливковые масла",
			"Национальные блюда",
			"Авторские овощные закуски"
		]
	},
	{
		id: "tigerLily",
		special: "tigerLily",
		title: 'TIGER LILY',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Tiger-Lily.png`,
		text: 'Наши гости могут насладиться разнообразными блюдами китайской кухни, приготовленными по оригинальным и традиционным рецептам.',
		subText1: 'доступно',
		subText2: '70 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'Tiger Lily — современный ресторан с азиатским уклоном, где сочетаются традиционные вкусы и новаторские подходы. Визитная карточка заведения это авторские роллы и острые тайские супы.',
		cardIcon1Desktop: iconRubDark,
		cardIcon2Desktop: iconCapacityDark,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 2 000 ₽',
		price: 'от 2 000 руб',
		tags: [
			{name: "#китайская" ,},
			{name: "#семейный" ,}
		],
		insideText1: "Шеф-повар Tiger Lily экспериментирует с ингредиентами, создавая уникальные сочетания. Ресторан регулярно проводит тематические ужины и мастер-классы по приготовлению суши.",
		insideText2: "Минималистичный дизайн с акцентами в стиле джунглей и мягким светом делает пространство идеальным для романтических свиданий и вечеринок.",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Tiger-Lily/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Tiger-Lily/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Tiger-Lily/icon_3.svg`,
        },
		subIconsText: [
			"Фирменные коктейли",
			"Паназиатская кухня",
			"Живая музыка"
		]
	},
	{
		id: "leth",
		special: "leth",
		title: 'LETH',
		image: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Leth.png`,
		text: 'Меню ресторана предлагает уникальные блюд, основное внимание уделяется игре вкусов и визуальным эффектам.',
		subText1: 'доступно',
		subText2: '30 мест',
		subText3: 'Средний чек',
		subText4: 'Вместимость ресторана',
		text1: 'LETH — это современный ресторан, где сочетаются инновационные кулинарные техники и изысканные вкусы. Наше меню вдохновлено мировыми тенденциями, а каждое блюдо — это настоящее произведение искусства.',
		cardIcon1Desktop: iconRubDark,
		cardIcon2Desktop: iconCapacityDark,
		cardIcon1Mobile: iconRubDark,
		cardIcon2Mobile: iconCapacityDark,
		price1: 'от 2 500 ₽',
		price: 'от 2 500 руб',
		tags: [
			{name: "#европейская" ,},
			{name: "#авторская" ,}
		],
		insideText1: "LETH славится уникальной атмосферой, где элегантность встречается с новаторством. Мы используем только лучшие сезонные продукты, чтобы создать неповторимые вкусовые сочетания. Ресторан регулярно проводит тематические ужины и коллаборации с известными шеф-поварами, предлагая гостям новые гастрономические впечатления.",
		insideText2: "Идеальное место для романтических вечеров, деловых встреч и особых мероприятий.",
        iconsInside: {
            icon1: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Leth/icon_1.svg`,
            icon2: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Leth/icon_2.svg`,
            icon3: `${process.env.PUBLIC_URL}/images/locations/Restraunts/Leth/icon_3.svg`,
        },
		subIconsText: [
			"Гастро-перформансы",
			"Молекулярные коктейли",
			"Меню-сюрприз"
		]
	}
]